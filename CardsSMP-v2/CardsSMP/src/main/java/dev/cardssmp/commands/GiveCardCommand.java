...
public class GiveCardCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (type == null) {
            ...
            sender.sendMessage(ChatColor.GRAY + "Valid cards: " + Arrays.stream(CardType.values())
        ...
        sender.sendMessage(ChatColor.GREEN + "Gave " + type.getId() + " card to " + target.getName() + "!");
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 1)
            ...
            Arrays.stream(CardType.values()).map(t -> t.name().toLowerCase()).forEach(completions::add);
